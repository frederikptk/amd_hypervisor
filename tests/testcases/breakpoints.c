...
int main() {
	...
	fuzz_fd= open(HYPERKRAKEN_FUZZ_PATH, O_RDWR);
	if (fuzz_fd == -1) {
		printf("Could not open " HYPERKRAKEN_FUZZ_PATH "\n");
		return EXIT_FAILURE;
	}
	...
	TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_SET_BREAKPOINTS, &breakpoints));

	// set the fuzzing counter region
	fuzz_mmap = mmap(NULL, 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED, fuzz_fd, 0);
	TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_SET_FUZZ_CNTR_REGION, &guest_id));
