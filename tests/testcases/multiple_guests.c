...
int main() {
	...
	ctl_fd = open(HYPERKRAKEN_PROC_PATH, O_RDWR);
	if (ctl_fd == -1) {
		printf("Could not open " HYPERKRAKEN_PROC_PATH "\n");
		return EXIT_FAILURE;
	}

    // Create two guests with VCPUs
	printf("Create guest 1\n");
	TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_IOCTL_CREATE_GUEST, &guest_id_1))
	id_data_1.guest_id = guest_id_1;
	TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_IOCTL_CREATE_VCPU, &id_data_1))

	printf("Create guest 2\n");
	//printf("&guest_id_2: 0x%lx\n", &guest_id_2);
	//printf("&id_data_2: 0x%lx\n", &id_data_2);
    TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_IOCTL_CREATE_GUEST, &guest_id_2))
	id_data_2.guest_id = guest_id_2;
	TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_IOCTL_CREATE_VCPU, &id_data_2))
	...
	TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_IOCTL_VCPU_RUN, &id_data_1))
    ...
    TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_IOCTL_DESTROY_GUEST, &guest_id_1))

    printf("Run guest 2\n");
    id_data_2.guest_id = guest_id_2;
	id_data_2.vcpu_id  = vcpu_id_2;
	// First exit: lazy pagefault of code
	TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_IOCTL_VCPU_RUN, &id_data_2))
	// Second exit: lazy pagefault of data @ 0x1000
	TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_IOCTL_VCPU_RUN, &id_data_2))
	// Third exit: HLT instruction as last executed instruction
	TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_IOCTL_VCPU_RUN, &id_data_2))
    ...
    TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_IOCTL_DESTROY_GUEST, &guest_id_2))
