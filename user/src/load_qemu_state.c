...
// Tell Hyperkraken to create a guest from the KVM trace
int64_t create_hyperkraken_fuzzing_guest(int ctl_fd) {
    uint64_t    guest_id = GUEST_CREATE_KVM_REC;

    TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_IOCTL_CREATE_GUEST, &guest_id))
...
int main(int argc, char** argv) {
    ...
    // Register the KVM tracing in Hyperkraken
    if (stop_qemu()) return EXIT_FAILURE;
    TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_BEGIN_KVM_RECORD,NULL))
    ...
    stop_qemu();
    TEST_IOCTL_RET(ioctl(ctl_fd, HYPERKRAKEN_END_KVM_RECORD, NULL))

    // Dump QEMU memory
    printf("Dumping QEMU memory to file\n");
    if (dump_qemu_mem()) return EXIT_FAILURE;

    // Now create the Hyperkraken guest with all loaded parameters
    printf("Create Hyperkraken guest\n");
    guest_id = create_hyperkraken_fuzzing_guest(ctl_fd);

    // Load the QEMU memory dump into Hyperkraken
